    ...
    div(class = 'panel panel-primary',
        ...
        div(class = 'panel-body',
          ...
          wellPanel(
            ...
            # run button
            actionButton("SE_run_algo", "Run", icon = icon('play', lib = 'glyphicon'))
          ) # panel body
        ) # algorithm panel
    ),
    ...
    div(class = 'panel panel-green',
        ...
        div(class = 'panel-body',
            ...
            selectInput('SE_choose_meta', 'Select past algorithm:',
                        choices = structure(as.list(seq_along(app_state$tsev$algo_meta)),
                                            names = name_vec(app_state$tsev$algo_meta)), 
                        selected = input$SE_choose_meta),
            {
            if(is.null(input$SE_choose_meta)) # no algorithm selected
              p('No metadata to show')
            else {
              meta <- app_state$tsev$algo_meta[[as.numeric(input$SE_choose_meta)]]
              tagList(
